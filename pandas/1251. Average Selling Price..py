...
def average_selling_price(
    ...
    res = res[
        (res["purchase_date"] >= res["start_date"])
        & (res["purchase_date"] <= res["end_date"])
        | pd.isna(res["purchase_date"])
    ]
    res = (res["price"] * res["units"]).groupby(res["product_id"]).sum() / res[
        "units"
    ].groupby(res["product_id"]).sum()
